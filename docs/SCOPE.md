...
| `experiment-engine` -- API surface, `estimator.py` (doubly-robust causal estimation) | Not started | The mSPRT core (`msprt.py`) is real and validated, see the table above. What doesn't exist yet: a clean caller-facing API wrapping `MSPRTConfig`/`MSPRTState` for actual use in an experiment, and the separate doubly-robust potential-outcomes estimator (`estimator.py`) the original design also named. Both are legitimate next pieces, neither blocks on the other. |
